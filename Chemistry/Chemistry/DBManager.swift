...
class DBManager: NSObject {
    ...
    {
        sharedInstance.database!.open()
        let resultSet: FMResultSet! = sharedInstance.database!.executeQuery("SELECT mol.IUPAC FROM mol WHERE mid = '\(setNum)'", withArgumentsIn: nil)
        //let setInfoArray : NSMutableArray = NSMutableArray()
        if (resultSet != nil)
        {
        ...
        sharedInstance.database!.close()
        return resultSet.string(forColumn: "IUPAC")
